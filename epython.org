...
#+begin_src emacs-lisp  ;;; epython.el --- Provides various extra utilities to python mode.
  ...
  "
    ...
    ;; Above we set set flake8 as the first checker in the chain.
    ;; Usually flake8 will then call pylint so we do not want
    ;; to explicitly add pylint as the next checker. But if for
    ;; some reason flake8 was *NOT* chaining into pylint, then you do
    ;;    (flycheck-add-next-checker 'python-pylint 'python-flake8)
    ;; to give flycheck a hint.
