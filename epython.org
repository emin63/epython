...
#+begin_src emacs-lisp  ;;; epython.el --- Provides various extra utilities to python mode.
  ...
  "
    ...
    ;; Rough seems like a faster replacement for flake8 (but not pylint)
    ;; so set that to be first instead of 'python-flake8

    (setq first-python-checker 'python-ruff)
    (setq flycheck-checker first-python-checker)
    (flycheck-add-next-checker first-python-checker 'python-pylint)
